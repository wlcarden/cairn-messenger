...
**Duress wipe deferred to v1.5.** A second passphrase that triggers irreversible destruction of on-device key material, modeled on the existing GrapheneOS duress-PIN pattern. Observably destructive rather than concealing.

**The architectural answer to compelled unlock is tier separation, which already exists in v1.** Master identity is Shamir-split among recovery peers and not present on the device; operational identity is on-device and exposed under unlock. The realistic bounded compromise is messages and contacts at the time of seizure, recoverable to a new operational identity post-event via the social-recovery process.
