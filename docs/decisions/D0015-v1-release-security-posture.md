...
- **May become distinct from Briar in v1.5** with the recruited pool addition combined with reproducible-builds-plus-rebuild attestation — conditional on recruitment outcomes producing a functional pool.

**Recruitment-outcome conditional framing (per consolidated external-reads triage M8 / M14).** The earlier brief versions framed v1.5 differentiation as automatic with the v1.5 ship-event. The consolidated external-reads triage maintainer review identified that the recruitment work producing zero reviewers at v1 ship (the condition that lets v1 ship without the pool) does not magically produce 5+ reviewers at v1.5 ship; v1.5 differentiation through a recruited pool is conditional on a recruitment outcome §8.6 explicitly names as outside the project's control. **The brief's longer-term differentiation argument therefore does not depend on the recruited pool forming.** If the pool forms at v1.5, the v1.5 stack lifts from "aligned with comparable tools" to "additionally distinct from them" through binary-equivalence multi-party verification; if the pool does not form (or forms at sub-architectural-target size), v1.5 ships with reproducible builds + F-Droid rebuild attestation alone, which is the same posture Briar already operates and which the brief explicitly frames as "Cairn's value proposition still holds at this posture." The recruitment-outcome is treated as a v1.5 bonus rather than a v1.5 differentiation prerequisite.

Cairn's v1 distinction lives in the integration plus the three remaining architectural commitments (identity tiers, trust graph, social recovery), not in the release-security stack. The brief frames this honestly per §4.3 update. **v1.5+ distinction continues to live in those same three commitments rather than depending on a recruited reviewer pool forming.**

### Sigsum witness threshold (cross-reference to Section 5.5)

The Sigsum witness cosignature requirement is an architectural property specified in Section 5.5 and cross-referenced here: minimum 3 witnesses, 2-of-3 acceptance threshold over any given log state (per consolidated external-reads triage C9 / M3). The Q5/Q10 witness-pool recruitment task is to identify three or more witnesses sufficient to satisfy this requirement; if recruitment cannot produce three witnesses, the v1 stack does not deliver Sigsum split-view protection and the project must either delay ship until recruitment closes or document explicitly that v1 ships without operational Sigsum witness coverage. This decision is on the record at v1 ship-readiness review per Section 5.5.
