...
1. **Release signing flow.** The CI-side OIDC token request, Sigstore signing event production, and Rekor entry posting are release-pipeline concerns, owned by the build infrastructure. v1 expects to use `cosign sign-blob` via the project's GitHub Actions / equivalent pipeline. _[Revised 2026-06-09 — D0042.]_ The `cosign sign-blob` expectation predates the landed Ed25519 `COSE_Sign1` manifest envelope; stock cosign is ECDSA-P256 + blob-format and cannot produce that. D0042 §3 resolves the fork by **adopting the Sigstore-native model** — the release manifest is now a detached ECDSA-P256 signature over the canonical-CBOR bytes (it leaves the `COSE_Sign1` envelope; messaging/trust-graph keep COSE), so stock `cosign sign-blob` produces it and any third party can `cosign verify-blob` it. This confirms the cosign expectation and adds the verify-side adaptations (Fulcio P-256 extraction, detached-sig verify, SCT, SAN-URI identity) in D0042 §6.
