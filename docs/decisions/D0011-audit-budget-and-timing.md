...
**Added back to pre-pilot scope per consolidated external-reads triage C15 / L3: Shamir-library timing-safety verification.** The cryptographer review's L3 finding identified that D0011's F5-narrowing argument ("well-understood primitive with extensively-audited reference implementations") is sound for the cryptographic construction but does not cover the specific library's timing-safety properties. The chosen Shamir library (vsss-rs candidate per Q8, or alternative per Q8 resolution) may have known timing side channels or low-threshold edge-case bugs that property tests cannot detect (a property test cannot detect a constant-time violation; a non-cryptographer source review cannot reliably catch subtle GF(256) timing-leak bugs). The pre-pilot audit scope therefore includes a narrowly-defined Shamir-library timing-safety verification task: the auditor verifies the chosen library has either (a) an external audit attesting timing-safety, or (b) a documented constant-time implementation strategy verifiable by benchmark (e.g., `cargo bench` showing flat timing across share inputs). Marginal cost in audit time: ~0.25–0.5 person-day (a small fraction of the 1–2 person-week pre-pilot envelope); meaningful coverage of a residual risk the F5 narrowing left uncovered.
...
- **Scope:** Full cryptographic-primitive review (re-confirming the pre-pilot audit findings against any v1.5 changes); full trust-graph operation handling; full recovery flow; capability-token construction; release-security stack including Sigstore, Rekor, Sigsum integrations and the v1.5 recruited reviewer pool operational verification (the pool's attestation toolkit, the v1.5 reproducible-build pipeline, and the binary-equivalence verification process per [D0015](D0015-v1-release-security-posture.md)); UnifiedPush push-notification implementation if push is enabled by default in v1.5.
- **Budget: $60-220K** reflecting market rates at the named firms. $60-150K band fits Cure53 or Quarkslab at the stated 4-6 week engagement; $180-220K covers Trail of Bits or NCC Group at the same scope at standard rates. Lower bound depends on auditor-subsidy programs (Open Tech Fund audit grants; Cure53 mission-org rates; Trail of Bits civic-tech engagements at reduced rate). Upper bound covers unsubsidized engagement at standard rates for a 4-6 week engagement.
