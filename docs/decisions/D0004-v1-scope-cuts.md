...
3. **Reproducible builds deferred to v1.5.** v1 ships with Sigstore identity-based signing and external source-code review (reviewers review the source, not the binary). Binary reproducibility is added in v1.5 once the toolchain is stable. The reviewer pool reviews source code in v1; their attestations are Sigsum-anchored regardless.
