...
>
> **Still deferred to the on-device / APK cycle (D0020 §1.1↔§1.9 revision when undertaken):** producing/bundling per-ABI `libsimplex.so` + the GHC runtime (~161 `.so`/ABI from SimpleX's official bundle — NOT buildable here, no Haskell cross-build) and linking the `cairn-uniffi` cdylib against them; the `cairn-uniffi` handle selecting `FfiSidecarTransport` on Android; routing the in-process daemon through the C-Tor SOCKS proxy (a post-init `/network socks=…` command, D0020 §2.2); an encrypted DB (`DbOpts::encrypted`); on-device runtime + multi-conversation demux. (The MSRV-bump prerequisite is DONE — see finding #2.) So ws-core remains the **desktop/dev/CI** transport (live-validated above); the Android FFI transport is **code-complete + host-runtime-proven + builds on the pinned toolchain**, pending those device-cycle steps.
