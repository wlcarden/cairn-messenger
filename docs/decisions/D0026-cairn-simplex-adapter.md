...
> 2. **MSRV correction: the FFI build needs rust ≈1.91, NOT 1.88.** `simploxide-sxcrt-sys 0.2.0`'s LIB uses `Duration::from_mins` (stable since 1.91, `duration_constructors`); its `build.rs` `let`-chains (the earlier "≥1.88" claim) were a _necessary-but-insufficient_ part. The workspace MSRV (1.88, D0018 §7.4) therefore **cannot build the Android target yet** — a FURTHER MSRV bump (≈1.91+) is the gating follow-on for the APK cycle. Host builds are unaffected (target-gated off).
>
...
>
> **Still deferred to the on-device / APK cycle (D0020 §1.1↔§1.9 revision when undertaken):** the workspace MSRV bump to ≈1.91; producing/bundling per-ABI `libsimplex.so` + the GHC runtime (~161 `.so`/ABI from SimpleX's official bundle — NOT buildable here, no Haskell cross-build) and linking the `cairn-uniffi` cdylib against them; the `cairn-uniffi` handle selecting `FfiSidecarTransport` on Android; routing the in-process daemon through the C-Tor SOCKS proxy (a post-init `/network socks=…` command, D0020 §2.2); an encrypted DB (`DbOpts::encrypted`); on-device runtime + multi-conversation demux. So ws-core remains the **desktop/dev/CI** transport (live-validated above); the Android FFI transport is **code-complete + host-runtime-proven**, pending those device-cycle steps.
