...
>
> **Still deferred (needs Tor wired OR is a v1.x lever):** a two-party on-device message (blocked on the Tor step below, not on the transport); routing the in-process daemon through the C-Tor SOCKS proxy (a post-init `/network socks=…` command, D0020 §2.2 — now **empirically confirmed required** by the on-device run below); an encrypted DB (`DbOpts::encrypted`). The `cairn-uniffi` handle's per-target transport selection is **DONE** (D0027 §2.4); the MSRV prerequisite is **DONE** (finding #2); the send-delivery-assurance + per-connection recv demux are **DONE** (above). So ws-core remains the **desktop/dev/CI** transport (live-validated above); the Android FFI transport is now **code-complete + host-runtime-proven + arm64-link-validated + APK-bundled + on-device-RUN-validated** (below).
>
