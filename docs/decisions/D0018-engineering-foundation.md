...
> **Toolchain bump 1.85.0 → 1.88.0 (2026-06-01 — the first deliberate rotation).** Triply-justified, all converging on rust ≥1.88: (1) the SimpleX **Android FFI transport** — `simploxide-sxcrt-sys`'s `build.rs` uses `let`-chains (stabilized 1.88) per D0026 §12; (2) the high-level `simploxide-client` (`const Duration::from_hours`, ~1.87+); (3) the **`time` RUSTSEC-2026-0009** fix (`time >= 0.3.47` needs 1.88). Workspace `rust-version` bumped to `1.88` in lockstep. Re-validation: all four CI gates (clippy `--all-targets --all-features -D warnings` / test / doc / fmt) pass on 1.88 across all 14 crates. Churn was minimal + mechanical: a removed `clippy::match_on_vec_items` allow (now covered by `indexing_slicing`), a `doc_link_code` doc-comment reword, and a `format_push_string` (nursery) workspace-allow (a tracked `write!`-conversion follow-up). Byte-stability discipline (§9.1) is unaffected by the rotation itself — exact crate pins unchanged in the toolchain commit; this is a compiler-toolchain rotation, re-validated. **Follow-on (same day, separate commit):** the bump unblocked the paired §9.1 pin event that cleared justification (3) — `serde =1.0.215 → =1.0.220` (its post-split `serde_core` is `time 0.3.47`'s declared floor) + `time 0.3.44 → 0.3.47`, with `serde_core`/`time-core`/`time-macros`/`num-conv` riding along (all MIT/Apache, single-version) — re-validated on all four gates + `cargo audit`; `RUSTSEC-2026-0009` is no longer accept-listed in `deny.toml` / `.cargo/audit.toml`. These are verify-/parse-path-only deps (no canonical-CBOR exposure), so byte-stability of the wire formats is preserved.
>
> **Correction (2026-06-01, justification (1)): 1.88 is necessary but NOT sufficient for the Android FFI transport.** When that transport was implemented + validated (D0026 §12), the `aarch64-linux-android` build surfaced that `simploxide-sxcrt-sys 0.2.0`'s LIB (not just its `build.rs`) uses `Duration::from_mins` — stable only since rust **1.91** (`duration_constructors`). The original "≥1.88 (build.rs let-chains)" reading undercounted because the feasibility probe ran on the default 1.94.1 toolchain, not the pinned 1.88. The 1.88 rotation stands (justifications (2) + (3) needed exactly 1.88, and 1.88 is a prerequisite for the FFI path), but **shipping the Android FFI transport required a FURTHER MSRV rotation to 1.91** — DONE same-day as the SECOND deliberate rotation (1.88 → 1.91; see `rust-toolchain.toml`). 1.91 is the verified minimal floor: `simploxide-sxcrt-sys` + the `cfg(target_os = "android")` `FfiSidecarTransport` compile on EXACTLY 1.91 for `aarch64-linux-android` ON THE PINNED TOOLCHAIN (they did NOT on 1.88). Re-validation: all four host gates pass on 1.91. Churn was minimal + mechanical (the 1.88→1.91 jump's new `clippy::all` lints): four `collapsible_if` → let-chain collapses, one `manual_is_multiple_of` (`% 2 != 0` → `!…is_multiple_of(2)`), and one `write!(..).expect()` → `let _ = write!(..)` (clippy 1.91 flags `expect()` on an infallible String write). Exact crate pins unchanged (byte-stability, §9.1).
