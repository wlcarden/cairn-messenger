...
- **Stage 3c — atomic re-split (≥2 devices, highest risk). LANDED + 2-Pixel-proven
  ...
  - **3c-c — multi-peer re-split UX. LANDED + 2-Pixel-proven over Tor.** Makes
    re-split a real user action, not just a driver hook, AND closes the multi-peer
    correctness gap. A new `RecoveryOwnerPeerStore` (over the existing
    `recovery_peers` category) records each contact we entrust a share to (on
    `entrustRecoveryShare`); `beginResplit` now gathers ALL of them and fans **one**
    fresh kit out atomically — `peer[i] → newCards[i]`, `num_shares = max(5,
peerCount)`, the rest are the owner's new paper backups. (Correctness, not
    polish: re-splitting per-peer would hand each peer a card from a different
    polynomial, which cannot reconstruct together — so it MUST be one kit to all
    peers.) An empty peer list falls back to the open-conversation peer, so the
    proven driver/2-Pixel path is unchanged. The "My identity" screen gains a
    **Refresh recovery shares** action → a card-entry dialog (paste a threshold of
    current cards) → a progress dialog (preparing → awaiting N acks → done, showing
    the new paper cards to write down / copy, or a non-leaking failure). A
    `ResplitUiState` StateFlow drives it; `dismissResplit` closes it. **Proof (oriole
    ↔ raven, over bundled Tor):** A entrusts a card to B (→ A records B in
    `RecoveryOwnerPeerStore`, B holds 154 B) → A leaves the conversation (`home`
    clears `connectionId`/`peerKeyRaw`, removing the open-peer fallback) → A re-splits
    (id `f4c00d32`) and the PREPARE reaches B **purely via the tracked list**
    (`PREPARE sent to 1 peer(s)`, NOT the fallback — had tracking failed it would log
    "No recovery peers yet") → B stages + ACKs → A tallies 1/1 + COMMITs → B
    **promotes to active, phrase preserved**. The driver `resplit` path calls the
    same `beginResplit` the UI button invokes; the Compose dialogs are pure
    `resplitUiState` rendering. (N>1 fan-out needs ≥3 devices, the §7 #302 blocker;
    the tracked-list gather + atomic COMMIT across the whole map is what's proven.)
