...
> **Revision 2026-06-01 — for ANDROID this is not a fallback, it is mandatory; and the FFI transport has LANDED (code-complete + host-runtime-proven).** Investigating on-device wiring (D0026 §12) found there is **no standalone Android `simplex-chat` CLI binary** — the official Android integration is in-process JNI `libsimplex`. So §1.1's CLI-subprocess sidecar is the **desktop / dev / CI** transport, and the §1.9 FFI-in-process path is the **only** Android transport (independent of the operational-reliability activation criterion above, which still governs desktop-style sidecars elsewhere). The §1.10 seam absorbs this as a second `SidecarTransport` impl: `cairn-simplex-adapter`'s `FfiSidecarTransport` (`#[cfg(target_os = "android")]`, on `simploxide-ffi-core`) shares the entire invitation/send/recv `flow` with the ws-core transport — they differ only at bring-up (in-process `init(DefaultUser, DbOpts)` vs a WebSocket dial). Validated 2026-06-01: a host runtime proof drove in-process `libsimplex` v6.5.3.0 (init → `/user` → real invitation link) + an `aarch64-linux-android` type-check. **APK-cycle status:** the workspace MSRV bump to 1.91 (D0018 §7.4) is **DONE**; the per-ABI `libsimplex` bundling + the cdylib link are **DONE** (2026-06-01) — `cargo ndk` links `libcairn_uniffi.so` against the official Android `libsimplex.so` (arm64-v8a only; STATIC-GHC, one ~191 MB lib) and the **210 MB arm64 APK bundles it** (`android/app/build.gradle.kts`). The Haskell cross-build is NOT in Cairn's pipeline — the lib is extracted from the official SimpleX APK (operator-provided, not committed). **Remaining: the on-device RUN needs a real arm64 device** — an x86_64 host cannot run it (QEMU2 refuses arm64 images on x86_64, and there is no x86_64 `libsimplex`). Tor routing is a post-init `/network socks=…` command (§2.2). Full detail: D0026 §12.
...
> **Revision 2026-06-02 — Android C-Tor LANDED + on-device-validated end-to-end (Pixel 6 / GrapheneOS).** Cairn now carries its OWN tor (`libtor.so`) — **no Orbot, no second app** — and a full messaging-over-Tor round-trip works on real v1 hardware. Findings that refine §2.1/§2.2:
>
> - **Integration substrate:** the Android engine is `info.guardianproject:tor-android` + `jtorctl`; its `org.torproject.jni.TorService` loads `libtor.so` and manages tor over a **Unix-domain ControlSocket** (cookie auth disabled — filesystem-permission-protected), NOT a TCP control port. So the §2.2 diagram's "`cairn-tor-transport` → SOCKS 9050 / control 9051" is the **desktop/CI** shape; on Android the in-process `libsimplex` (D0026 §12 FFI transport) consumes the SOCKS proxy directly via a `/network socks=127.0.0.1:<port>` command (the wiring of commit 36f2e39), and tor lifecycle/control is `TorService` + jtorctl. This is the SAME CLI→in-process-FFI shift the SimpleX integration took (§1.9 revision) — the `Transport` seam holds; the Android engine differs, the SOCKS contract above it is identical. `cairn-tor-transport`'s hand-rolled SOCKS5 + SAFECOOKIE control client (D0025) remains the **desktop/dev** path.
> - **Version pin: `tor-android:0.4.8.19`, NOT the §2.1 "0.4.9.8+" floor.** The 0.4.9.8 AAR sets `minCompileSdk=37`, which would force AGP ~8.13+/9.x and break the load-bearing Willir `cargo-ndk` plugin 0.3.4 (project is on AGP 8.5.1 / compileSdk 34). 0.4.8.19 (`minCompileSdk=1`) wraps a current 0.4.8.x-LTS tor with no client-relevant security delta; revisit 0.4.9.x when the cargo-ndk toolchain is modernised.
> - **On-device proof:** the 232 MB arm64 APK bundles `libtor.so` (9.2 MB) next to `libsimplex.so` + `libcairn_uniffi.so`; on launch `TorService` boots tor, which **Bootstrapped 0% → 100% in ~8 s**, and the in-process `libsimplex` selftest routed `/_connect` through it and returned a real `{"type":"invitation",…}` SimpleX link **over Tor** (the `BROKER`-via-`.onion` that direct connections returned is resolved through the bundled Tor). `MainActivity` binds `TorService`, polls `status/circuit-established` via jtorctl, reads `getSocksPort()`, and drives the selftest.
> - **Environment finding (NOT a Cairn defect):** the testbench Pixel is a CrypTAK tactical device with an always-on **Tailscale VPN lockdown** (`always_on_vpn_lockdown=1`) that admin-prohibits (EACCES) all non-VPN outbound; Cairn's direct Tor-relay connections were blocked until the operator added Cairn to `always_on_vpn_lockdown_whitelist` + re-armed the VPN. On a non-locked device this step is unnecessary. The engine + integration were correct throughout — the gap between "tor runs" and "tor bootstraps" was entirely this device-policy block.
> - **Still deferred:** Lyrebird/obfs4 PT bundling (§2.3 — only needed for censored networks that block direct relay access; the clearnet-relay bootstrap above does not need it); the production `remoteMessaging` ForegroundService lifecycle (§2.5 — the pipeline-proof Activity hosts the bound service for now).
